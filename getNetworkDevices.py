...
async def get_devices(aiomeraki: meraki.aio.AsyncDashboardAPI, network):
    try:
        ...
        # Create the devices MongoDB data
        if devices:
            connect_mongo(data='NetworkDevices', collection=f'{network["name"]} ', content=devices)  
    return network["name"]
...
async def main():
    ...
    async with meraki.aio.AsyncDashboardAPI(
        ...
        networkClientsTasks = [get_devices(aiomeraki, net) for net in networks]
